...
def get_num_of_rows_follow(single_row):
    ...
    single_entry = [hurricane[0][0:2],hurricane[0][2:4],hurricane[0][4:8],hurricane[1].strip(),long(hurricane[2])]
    report_hurricane_wise.append(single_entry)
    return long(hurricane[2])
...
def model_prep(dataName,prediction_value,start,end):
    print(len(dataName))
    X = dataName[start].values.reshape(len(dataName),-1)
    Y = dataName[end]
    print(X.shape)
    print(Y.shape)
    lm = LinearRegression()
...
def generate_new_dataset(lat,lag,region,index=0):
    ...
    lag_end = int(lag) + 2
    if region == 1:
        new_latt = atlantic_latt.loc[lambda atlantic_latt : atlantic_latt[0].astype(dtype=float,errors="ignore")>=lat_start,:]
        new_latt = new_latt.loc[lambda (new_latt):new_latt[0].astype(float)<=lat_end,:]
        new_latt = new_latt.dropna(axis=1,how="all")

        new_lag = atlantic_long.loc[lambda (atlantic_long):atlantic_long[0].astype(dtype=float,errors="ignore")>=lag_start,:]
        new_lag = new_lag.loc[lambda (new_lag):new_lag[0].astype(dtype=float,errors="ignore")<=lag_end,:]
        new_lag = new_lag.dropna(axis=1,how="all") 
        
    elif region == 2:
        new_latt = pacific_latt.loc[lambda (pacific_latt):pacific_latt[0].astype(dtype=float,errors="ignore")>=lat_start,:]
        new_latt = new_latt.loc[lambda (new_latt):new_latt[0].astype(dtype=float,errors="ignore")<=lat_end,:]
        new_latt = new_latt.dropna(axis=1,how="all")

        new_lag = pacific_long.loc[lambda (pacific_long):pacific_long[0].astype(dtype=float,errors="ignore")>=lag_start,:]
        new_lag = new_lag.loc[lambda (new_lag):new_lag[0].astype(dtype=float,errors="ignore")<=lag_end,:]
        new_lag = new_lag.dropna(axis=1,how="all") 
...
def user_input():
	...
	path1.append(path_lat[0])
        path1.append(path_log[0])
...
def get_path():
    ...
    for i in range(0,len(path4),2):
        path2.append(path4[i])
        path3.append(path4[i+1])
    print(path2)
    print(path3)
    plt.plot(path2,path3)
    for j in range(0,len(path2)):
        plt.plot([path2[j]],[path3[j]])
    plt.show()
